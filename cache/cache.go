...
// UseCache
// Deprecated: 设置本次查询使用缓存 单次逻辑有BUG，设置无效，当没有在Table里面配置但是临时启用缓存时，创建数据不会清理过期的缓存，会导致脏读。
...
// DisableCache
// Deprecated: 设置本次查询不使用缓存 单次逻辑有BUG，设置无效，当Update是禁用cache时，Update不会清理过期的缓存，会导致脏读。
